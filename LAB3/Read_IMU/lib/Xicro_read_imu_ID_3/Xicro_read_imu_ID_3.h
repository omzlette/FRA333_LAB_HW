// ***************************************************************************************************************************************************//      |              This script was auto-generated by generate_library.py which received parameters from setup_xicro.yaml                   |//      |                                         EDITING THIS FILE BY HAND IS NOT RECOMMENDED                                                 |// ***************************************************************************************************************************************************#ifndef XICRO_READ_IMU_ID_3_H#define XICRO_READ_IMU_ID_3_H#include "Arduino.h" 
class Xicro{
    public:
        Xicro();
        void begin(Stream *SerialObject);
        void Spin_node();void Test();
        
        // gen public pub void        void publish_Imu_arduino(int32_t header__of__stamp__of__sec ,uint32_t header__of__stamp__of__nanosec ,String header__of__frame_id ,float orientation__of__x ,float orientation__of__y ,float orientation__of__z ,float orientation__of__w ,float *orientation_covariance ,float angular_velocity__of__x ,float angular_velocity__of__y ,float angular_velocity__of__z ,float *angular_velocity_covariance ,float linear_acceleration__of__x ,float linear_acceleration__of__y ,float linear_acceleration__of__z ,float *linear_acceleration_covariance );
        // gen        // gen state_srv_action        struct{        } State;        // gen struct srv client res        // gen service call void
    // get    
    private:
        uint8_t _Idmcu=3;
        Stream* _serial;
        uint8_t _getcrc(uint8_t crc,uint8_t *data,uint16_t len);
        const uint8_t _CRC_8_TABLE[256] = {};
        uint8_t _start[4]={73,109,64,99};
        uint8_t _stop[2]={126,126};
        uint8_t _continue[2]={42,42};
        void _Sendstart();
        void _SendSignature(uint8_t Idmcu,uint8_t Mode);
        void _Sendstop();
        void _Sendcontinue();
        void _SendUint8(uint8_t *data,uint8_t len);
        void _SendUint16(uint16_t *data,uint8_t len);
        void _SendUint32(uint32_t *data,uint8_t len);
        void _SendUint64(uint64_t *data,uint8_t len);
        void _SendInt8(int8_t *data,uint8_t len);
        void _SendInt16(int16_t *data,uint8_t len);
        void _SendInt32(int32_t *data,uint8_t len);
        void _SendInt64(int64_t *data,uint8_t len);
        void _SendFloat32(float *data,uint8_t len);
        void _SendString(String *data,uint8_t len);
        void _SendBool(bool *data,uint8_t len,bool flagNext);
        void _SendIdTopic(uint8_t IdTopic);


        uint8_t _crc=0;
        void _Sendcrc();
        uint8_t _state=0;
        uint8_t _datain[1]={0};
        uint64_t _bb=0;
        
        
        uint8_t _incomingIndexTopic=0;
        int8_t _OnTopic=-1;
        uint8_t _Ontype=0;
        uint8_t _Indexdata=0;
        uint8_t _crcIn=0;
       

        String _bufff="";
        uint8_t q=0;
        uint8_t w=0; 
        uint8_t _loop=0;
        uint8_t _loopTo=0;
        // gen        void* _nonverify[0][0][0];        void* _verify[0][0][0];        uint8_t _Idtopic_sub[0]={};        uint8_t _TopicType[0][0]={};        uint8_t _Totalvar[0]={};        uint8_t _Nofdata[0][0]={};        // gen

        uint8_t _Onindex=0;
        uint8_t _Ongrab=0;
        uint8_t _OngrabTo=0;
        uint8_t _statetogo=0;
        uint8_t _OnMode=0;
};
#endif